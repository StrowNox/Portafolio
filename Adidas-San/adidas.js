        const frases = ["FREE STANDARD SHIPPING WITH ADICLUB", "FAST, FREE DELIVERY WITH PRIME", "FREE BACKPACK WITH ORDERS $150+"];
        let i = 0;
        const texto = document.getElementById("texto");

        // Duración de la transición de opacidad en CSS (en milisegundos)
        ...
        setInterval(() => {
            ...
            // 2. Espera a que el desvanecimiento termine, luego cambia el texto y lo hace aparecer (fade-in)
            setTimeout(() => {
                i = (i + 1) % frases.length; // Avanza a la siguiente frase
                texto.textContent = frases[i]; // Cambia el contenido del texto
                texto.style.opacity = 1; // Hace que el texto aparezca (fade-in)
            }, transitionDuration); // Espera la duración de la transición
        }, 3000); // Intervalo total: tiempo de visualización + 2 * duración de transición
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    botonAnadirBolsa.addEventListener('click', () => {
        if (tallaSeleccionada) {
            ...
            const nombreProducto = 'SAMBA OG SHOES - White'; // <-- CAMBIA ESTO (ej. 'SAMBA OG SHOES - Brown')
            const imagenProducto = 'https://assets.adidas.com/images/h_2000,f_auto,q_auto,fl_lossy,c_fill,g_auto/3bbecbdf584e40398446a8bf0117cf62_9366/Samba_OG_Shoes_White_B75806_01_00_standard.jpg'; // <-- CAMBIA ESTO
